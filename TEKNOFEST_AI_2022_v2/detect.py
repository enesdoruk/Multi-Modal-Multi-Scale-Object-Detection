...
from yolo.utils.general import (LOGGER, check_img_size, non_max_suppression, scale_coords, strip_optimizer)
...
@torch.no_grad()
def run(image, weights='weights/zekai.pt',
        imgsz=[1536], device='0', augment=False,  
        update=False, half=False):
    ...
    iou_thres = 0.45
    agnostic_nms =False
    pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=100)
    dt[2] += time_sync() - t3
